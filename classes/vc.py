...
class VoiceCloningService(AIModelService):
    ...
    def get_filtered_axons(self):
        try:
            ...
            queryable_uid = queryable_uids * torch.Tensor([any(self.metagraph.neurons[uid].axon_info.ip.startswith(prefix) for prefix in ['213.173,98.','213.173,99.'])for uid in uids])
